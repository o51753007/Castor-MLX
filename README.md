...
本專案是一個「以人為導向、AI 為工具」開發的快速原型，目標是在 M3 晶片上達成純粹、高效且極度隱私的本地 LLM 使用體驗。
...
* **環境管理**: 使用 `uv` 進行相依性鎖定與虛擬環境管理。

---

## 🌟 技術實作特色

### 1. 資源防禦機制 (Resource Guard)
針對 MacBook Air 續航優化。當使用者關閉瀏覽器分頁時，後端透過 `request.is_disconnected()` 立即停止 GPU 運算，確保電力不被浪費。

### 2. 高效推理循環 (Efficient Inference)
直接調用 `mlx_lm.stream_generate`。針對 Apple Silicon 的統一記憶體架構（UMA）進行調校，在 M3 晶片上實現極低延遲。

### 3. 輕量化持久層
無資料庫設計。透過 JSON 序列化將紀錄存於 `data/sav/`，並自動過濾 `system_prompt` 優化顯示邏輯。
...
│   └── sav/            # 本地對話紀錄 (JSON)
├── models/             # 模型權重目錄
│   └── current/        # 符號連結 (Symbolic Link)，指向當前使用的模型
├── config.json         # 系統參數配置
...
### 1. 環境準備
確保設備為 MacBook Air M3 (16G RAM 以上)，並安裝 `uv`。
```bash
# 安裝相依套件
uv sync
```
